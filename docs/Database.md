...
| `square_variant_id` | `text` |  Nullable |
| `is_square_enabled` | `bool` |  |
...
| `square_payment_link_id` | `text` |  Nullable |
...
|------|------|-------------|
| `key` | `text` | Primary |
| `window_start` | `int8` | Primary |
| `count` | `int4` |  |
| `expires_at` | `timestamptz` |  |

## Table `exp_square_webhook_events`
...
## Table `exp_shippo_webhook_events`
...
| Policy | Command | Roles | Action | USING | WITH CHECK |
|--------|---------|-------|--------|-------|------------|
| `public_read_product_option_values` | SELECT | public | PERMISSIVE | `(EXISTS ( SELECT 1    FROM (exp_product_options po      JOIN exp_products p ON ((p.id = po.product_id)))   WHERE ((po.id = exp_product_option_values.option_id) AND (p.is_active = true) AND (p.is_archived = false))))` | — |
...
| Policy | Command | Roles | Action | USING | WITH CHECK |
|--------|---------|-------|--------|-------|------------|
| `public read visible future products` | SELECT | anon, authenticated | PERMISSIVE | `(is_visible = true)` | — |

### `exp_future_product_statuses`

| Policy | Command | Roles | Action | USING | WITH CHECK |
|--------|---------|-------|--------|-------|------------|
| `public read visible future product statuses` | SELECT | anon, authenticated | PERMISSIVE | `(is_visible = true)` | — |
