...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase
      ...
      .select('id, status, customer_name, customer_email, item_type, quantity, description, files, quote_amount, square_payment_link_url, quote_sent_at, quote_expires_at, quote_last_resent_at, quote_resend_count, production_handoff_at, recovery_reminder_sent_at, created_at, updated_at')
