...
export function ProductConfigurator({ product }: ProductConfiguratorProps) {
  ...
  const isValid =
    ...
    requiredOptions.every((o) => state.optionValues[o.option_key]?.trim().length) &&
    !isOutOfStock

  const handleVariantChange = (variantId: string) => setState((prev) => ({ ...prev, variantId }))
  const handleOptionChange = (key: string, value: string) => setState((prev) => ({
    ...prev, optionValues: { ...prev.optionValues, [key]: value },
  }))
  ...
  const handleQuantityChange = (delta: number) => setState((prev) => {
  ...
  const handleAddToCart = () => {
    ...
    const selectedOptions = options
      .map((opt) => {
        ...
        return { key: opt.option_key, label: opt.label, value: selectedVal, valueLabel: selectedValueMeta?.label ?? selectedVal }
      })
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {variants.length > 0 && (
        <Box>
          <FormLabel sx={{ display: 'block', mb: 1.25, fontWeight: 600, fontSize: '0.875rem', color: brandTokens.parchment }}>Size / Type</FormLabel>
          <Box sx={{ display: 'flex', gap: 1.25, flexWrap: 'wrap' }}>
            {variants.map((v) => (
              <VariantPill key={v.id} variant={v} selected={state.variantId === v.id} onClick={() => handleVariantChange(v.id)} />
            ))}
          </Box>
        </Box>
      )}
      ...
      {options.map((opt) => (
        <OptionField key={opt.id} option={opt} value={state.optionValues[opt.option_key] ?? ''} onChange={(val) => handleOptionChange(opt.option_key, val)} />
      ))}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          >−</Box>
          <Typography sx={{ minWidth: 28, textAlign: 'center', fontWeight: 600 }}>{state.quantity}</Typography>
          <Box component="button" onClick={() => handleQuantityChange(1)} aria-label="Increase quantity"
      ...
      {isReadyMade && (
        <Box sx={{ mt: -0.5, p: 1, borderRadius: 1, border: `1px solid ${alpha(brandTokens.parchment, 0.16)}`, backgroundColor: alpha(brandTokens.bgSurface, 0.52) }}>
          <Typography sx={{ fontSize: '0.75rem', fontWeight: 600 }}>Inventory: {
            product.inventory_status === 'forced_out_of_stock' ? 'Out of stock (manual override)'
              : product.inventory_status === 'forced_in_stock' ? 'In stock (manual override)'
              : product.inventory_status === 'low_stock' ? `Low stock (${product.inventory_qty ?? 0} left)`
              : product.inventory_status === 'out_of_stock' ? 'Out of stock'
              : product.inventory_status === 'untracked' ? 'In stock (not tracked)'
      ...
      {state.selectedProcessKeys.includes('nfc_tag') && (
        <Box>
          <FormLabel sx={{ display: 'block', mb: 1.25, fontWeight: 600, fontSize: '0.875rem', color: brandTokens.parchment }}>NFC Configuration</FormLabel>
          <Box sx={{ display: 'flex', gap: 2, flexDirection: { xs: 'column', md: 'row' } }}>
            ...
            {state.nfcEnabled && (
              <>
                <TextField label="Initial Data to Encode" placeholder="https://... or plain text" value={state.nfcTargetData}
                  onChange={(e) => setState((prev) => ({ ...prev, nfcTargetData: e.target.value }))}
                  size="small" fullWidth inputProps={{ maxLength: 250 }}
                  sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: alpha(brandTokens.parchment, 0.18) } }}
                />
                <FormControlLabel
        ...
        <Box>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            {process_pricing.map((process) => {
              ...
              return (
                <ProcessPill key={process.process_type_key} processTypeKey={process.process_type_key}
                  label={process.process_type_key.replace(/_/g, ' ')} priceDelta={process.price_delta}
                  selected={isSelected} onClick={() => handleProcessToggle(process.process_type_key)} />
              )
            })}
          </Box>
          {pricing.activeComboDiscount && (
            <Box sx={{ mt: 1.5, p: 1.25, borderRadius: 1, border: `1px solid ${alpha(brandTokens.forgeGold, 0.32)}`, backgroundColor: alpha(brandTokens.forgeGold, 0.1) }}>
              <Typography sx={{ fontSize: '0.78rem', fontWeight: 700, color: brandTokens.forgeGold, mb: 0.35 }}>Process Combo Discount Applied</Typography>
              <Typography sx={{ fontSize: '0.76rem', color: alpha(brandTokens.parchment, 0.72) }}>{formatComboDiscountLabel(pricing.activeComboDiscount)} — You save ${pricing.processDiscount.toFixed(2)}.</Typography>
            </Box>
          )}
        </Box>
      )}
      ...
      {product.has_designer && product.designer_mockup_url && (
        <>
          <Button variant="outlined" size="large" fullWidth onClick={() => setDesignerOpen(true)}
            sx={{ py: 1.25, fontSize: '0.9rem', borderColor: alpha(brandTokens.forgeGold, 0.4), color: brandTokens.forgeGold }}>
            Customize Design
          </Button>
      ...
      <Box sx={{ display: 'flex', gap: 1.5, flexDirection: { xs: 'column', sm: 'row' } }}>
        <Button variant="contained" size="large" fullWidth startIcon={<AddShoppingCartIcon />} onClick={handleAddToCart} disabled={!isValid}
          sx={{ py: 1.75, fontSize: '1rem', fontWeight: 700, letterSpacing: '0.06em', backgroundColor: isValid ? brandTokens.forgeGold : undefined,
            color: isValid ? brandTokens.bgVoid : undefined, '&:hover': { backgroundColor: isValid ? '#E8B84A' : undefined } }}>
          {isOutOfStock ? 'Out of Stock' : addedToCart ? '✓ Added to Cart' : 'Add to Cart'}
        </Button>
        {isSquareEnabled && squareItems.length > 0 && <SquareCheckoutButton items={squareItems} disabled={!isValid} />}
      </Box>
...
function ProcessPill({ processTypeKey, label, priceDelta, selected, onClick }: ProcessPillProps) {
  const emojiMap: Record<string, string> = { engraving_cutting: '🔥', printing: '🖨️', sublimation: '🌈' }
  const emoji = emojiMap[processTypeKey] ?? '⚙️'
  const deltaLabel = priceDelta > 0 ? ` (+$${priceDelta.toFixed(2)})` : ''

  return (
    <Box component="button" onClick={onClick} aria-pressed={selected}
      sx={{ px: 1.5, py: 0.75, borderRadius: 1, border: `1px solid ${selected ? brandTokens.forgeGold : alpha(brandTokens.parchment, 0.18)}`,
...
function VariantPill({ variant, selected, onClick }: VariantPillProps) {
  const deltaLabel = variant.price_delta > 0 ? ` (+$${variant.price_delta.toFixed(2)})` : ''
  return (
    <Box component="button" onClick={onClick} aria-pressed={selected}
      sx={{ px: 2, py: 0.875, borderRadius: 1, border: `1px solid ${selected ? brandTokens.forgeGold : alpha(brandTokens.parchment, 0.18)}`,
        background: selected ? alpha(brandTokens.forgeGold, 0.12) : 'none', color: selected ? brandTokens.forgeGold : brandTokens.parchment,
...
function OptionField({ option, value, onChange }: OptionFieldProps) {
  const labelSx = { display: 'block', mb: 1, fontWeight: 600, fontSize: '0.875rem', color: brandTokens.parchment }
...
function getStickerSizeGuidance(optionKey: string, value: string): string | null {
  ...
  switch (value) {
    case '1x1': return 'Expect at least 24 stickers per sheet at 1"x1".'
    case '2x2': return 'Expect at least 12 stickers per sheet at 2"x2".'
    case '3x3': return 'Expect at least 9 stickers per sheet at 3"x3".'
    case '4x4': return 'Expect at least 4 stickers per sheet at 4"x4".'
    case 'custom': return 'Custom size selected. Final sticker count per sheet will be confirmed during proofing.'
    default: return null
  }
}