-- -----------------------------------------------------------------------------
-- Migration 049: Complete schema reconstruction in final state (post-001-058)
--
-- This migration creates ALL tables, functions, indexes, triggers, RLS policies,
-- and privilege grants in their final state for a fresh database. It is
-- idempotent and safe to re-run.
--
-- NOTE: This file was restored from supabase/verification/schema_repair.sql
-- during the Batch 2 security remediation. It incorporates migrations 001-058
-- plus the Batch 2 fixes (weight_lb columns, dropped promo public-read policies,
-- private design-artifacts bucket, dropped legacy customer tables, etc.).
-- -----------------------------------------------------------------------------
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  on conflict (key, window_start) do update
    set count = case
      when exp_rate_limit_windows.expires_at < p_expires_at then 1
